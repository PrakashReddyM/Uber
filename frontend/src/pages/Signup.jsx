...
const Signup = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setAuthUser(data);
            setFormData({ name: '', email: '', password: '', location: null });
            navigate('/');
        } catch (error) {
        ...
        <div className="flex max-h-screen justify-center items-center">
            <div className="w-1/3 p-6 mt-10">
                <form onSubmit={handleSubmit} className="rounded px-8 pt-6 pb-8 mb-4">
                    ...
                    <p className="text-center font-bold text-xs  mt-4">OR</p>
                    <Link
                        to="/captain-signup"
                        className="bg-gray-950 text-[15px] w-full hover:bg-gray-800 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline mt-2 block text-center"
                    >
                        Move to Captain Account
                    </Link>
                </form>
            </div>
